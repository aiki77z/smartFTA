...
def extract_entities_incremental(chunks: List[Dict], output_file: str, print_raw_text: bool = False) -> List[Dict]:
    """
    增量提取实体，避免重复处理已处理的chunk
    """
    ...
    with open(output_file, "a", encoding="utf-8") as f:
        for chunk in chunks:
            ...
            try:
                ...
                print(f"提取有效实体数：{len(valid_entities)}")
...
def main():
    parser = argparse.ArgumentParser(description='实体识别和合并工具')
    ...
    if not args.skip_entity_extraction:
        print(f"开始实体提取，结果将保存到: {args.output_entities}")
        entities_results = extract_entities_incremental(chunks, args.output_entities, args.print_raw_text)
