...
class RepairAgent:
    ...
    def run(
        ...
        *,
        ...
        persist_draft_artifact: bool = False,
    ) -> Dict[str, Any]:
        ...
        if apply_patch and patch.get("operations"):
            ...
            if patch_application.get("changed"):
                ...
                patched_tree_source = "repair_patch"

        legacy_repaired_tree = None
        repair_error = None
        should_try_legacy = bool(apply_legacy_repair and repairable_issues and patched_tree is None)
        ...
        patch["repaired_tree_source"] = patched_tree_source
        if patched_tree is not None:
            patch["status"] = "patched" if patched_tree_source == "repair_patch" else "legacy_repaired"
        ...
        if persist_artifact and run_id:
            ...
            if persist_draft_artifact and patched_tree is not None:
                ...
                if patched_validation_report is not None:
                    ...
                    validation_artifact = put_agent_artifact(
                        ...
                        metadata={"producer": self.name, "source": patched_validation_report.get("source")},
                        producer=self.name,
                    )
        ...
        remaining_error_count = (
            ...
            if patched_validation_report is not None
            else (0 if patched_tree is not None else int(validation_report.get("error_count") or 0))
        )
        return {
            "agent": self.name,
            "artifact_type": ARTIFACT_REPAIR_PATCH,
            ...
            "human_review_required": bool(blocking_errors) or remaining_error_count > 0,
        }
